...
export async function crearNotaClinicaAction(pacienteId: string, data: any) {
  ...
  // 2. Crear nota_clinica — consultas no referencia historia_clinica directamente,
  //    lo hace a través de nota_clinica (historia_clinica → nota_clinica → consultas).
  const { data: notaClinica, error: notaError } = await supabase
    .from("nota_clinica")
    .insert({ historia_clinica_id: hc.id, estado: "En Consulta", created_at: new Date().toISOString() })
    .select("id")
    .single();

  if (notaError || !notaClinica) {
    console.error("Error creando nota_clinica:", notaError);
    return { error: "No se pudo crear la nota clínica." };
  }
