...
export function EditarConsultaModal({ consulta, pacienteId, onClose, onSaved }: {
 ...
 <div className="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center text-amber-600 shrink-0">
          ...
          <div>
 <p className="text-[14px] font-semibold text-slate-900">Editar consulta</p>
 <p className="text-[11px] text-slate-400">Modificar registro clínico actual</p>
          </div>
        </div>
      }
      footer={
 ...
 className="text-[11px] font-semibold text-amber-600 hover:underline flex items-center gap-1 border-0"
            >
          ...
          <div className="flex flex-col gap-3">
            <div 
              onClick={() => fileRef.current?.click()}
 className="border-2 border-dashed border-slate-200 rounded-xl p-5 flex flex-col items-center justify-center text-center text-slate-400 bg-slate-50 cursor-pointer hover:bg-slate-100 transition-colors"
            >
              <Icon name="cloud_upload" size={24} className="mb-2 opacity-50" />
 <p className="text-[13px] font-medium text-slate-600 mb-0.5">Haz clic aquí para añadir nuevos archivos</p>
            ...
            {(archivosExistentes.length > 0 || newFiles.length > 0) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                {archivosExistentes.map((a) => {
                  ...
                  return (
 <div key={`ext-${a.id}`} className="border border-slate-200 rounded-xl flex flex-col relative bg-white shadow-sm">
 <div className="relative h-28 bg-slate-50 flex items-center justify-center border-b border-slate-100">
                        {isImage ? (
 ...
 className="absolute top-1.5 right-1.5 w-7 h-7 rounded-lg bg-white/90 text-red-500 flex items-center justify-center shadow-md hover:bg-red-50 transition-colors"
 ...
 <p className="text-[12px] font-semibold text-slate-800 truncate" title={a.nombre_archivo}>{a.nombre_archivo}</p>
                        <div className="flex items-center gap-1">
 <span className="text-[10px] font-medium px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md border border-slate-200/60">
                            {tipoNombre}
                          </span>
                        </div>
                        {a.descripcion && (
 <p className="text-[11px] text-slate-500 mt-1 line-clamp-2">{a.descripcion}</p>
                        )}
                      </div>
                    </div>
                  );
                ...
                {newFiles.map((f, i) => {
                  ...
                  return (
 <div key={`new-${i}`} className="border border-amber-300 rounded-xl flex flex-col relative bg-amber-50/40 shadow-sm">
 <div className="relative h-28 bg-amber-100/50 flex items-center justify-center border-b border-amber-200/60">
                        {isImage ? (
 ...
 <p className="text-[12px] font-semibold text-slate-800 truncate" title={f.name}>{f.name}</p>
 ...
 className="flex items-center gap-1.5 px-3 py-2.5 w-fit text-[13px] font-medium text-amber-600 hover:bg-amber-50 rounded-xl transition-colors min-h-10">
