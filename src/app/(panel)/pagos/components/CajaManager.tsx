...
export function CajaManager({ mediosPago }: { mediosPago: { id: number; nombre: string }[] }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 p-6">
      <div className="max-w-md w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
        <div className="w-12 h-12 rounded-xl bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400 flex items-center justify-center mx-auto mb-4">
          <Icon name="point_of_sale" size={24} />
        </div>
        <h2 className="text-xl font-bold text-center text-slate-900 dark:text-slate-100 mb-2">Turno de Caja Cerrado</h2>
        ...
        <div className="space-y-4 mb-6">
          <h3 className="text-[13px] font-semibold text-slate-800 dark:text-slate-200">Saldos iniciales por medio de pago</h3>
          {mediosPago.map(m => (
            <div key={m.id} className="flex items-center gap-3">
              <span className="w-1/2 text-[13px] text-slate-600 dark:text-slate-300">{m.nombre}</span>
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-[12px] font-medium text-center">
            {error}
          </div>
        )}
