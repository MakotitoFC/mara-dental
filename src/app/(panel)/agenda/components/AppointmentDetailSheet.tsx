  ...
  return (
          ...
          {!confirmDelete ? (
            <>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => onEdit(cita)}
                  className="flex-1 h-11 flex items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 text-[13px] font-semibold transition-colors"
                >
                  <Icon name="edit" size={15} /> Editar
                </button>
                <button
                  onClick={() => setConfirmDelete(true)}
                  className="flex-1 h-11 flex items-center justify-center gap-2 rounded-xl border border-red-200 dark:border-red-900 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 text-[13px] font-semibold transition-colors"
                >
            ...
            <div className="rounded-xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/40 p-3 flex flex-col gap-2.5">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 h-9 rounded-lg border border-slate-200 dark:border-slate-700 text-[12px] font-medium text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 transition-colors"
                >
                  Cancelar
                </button>
                <button
    ...
    >
      ...
      <div className="flex items-center gap-3 pt-2 pb-4">
        ...
        <div className="min-w-0 flex-1">
          <button onClick={goToPatient} className="text-left w-full">
            <p className="text-[16px] font-bold text-slate-900 dark:text-slate-100 truncate hover:text-cyan-700 dark:hover:text-cyan-400 transition-colors">{cita.paciente_nombre}</p>
          </button>
          <p className="text-[12px] text-slate-500 dark:text-slate-400 capitalize truncate">{dateLabel}</p>
        </div>
      </div>
      ...
      <div className="relative mb-4">
        ...
        {statusOpen && (
          <div className="absolute top-full left-0 mt-1.5 z-10 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg overflow-hidden min-w-[170px]">
            {ESTADO_ORDER.map(e => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[12.5px] font-medium text-slate-700 dark:text-slate-300">{ESTADO_CITA_LABEL[e]}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-700 overflow-hidden">
          <div className="flex items-center gap-3 px-3.5 py-3">
            <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900/50 flex items-center justify-center shrink-0">
              <Icon name="schedule" size={15} className="text-slate-400 dark:text-slate-500" />
            </div>
            <div className="min-w-0">
              <p className="text-[10.5px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Horario</p>
              <p className="text-[13.5px] font-bold text-slate-900 dark:text-slate-100">{cita.hora_inicio} – {cita.hora_fin}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 px-3.5 py-3">
            <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900/50 flex items-center justify-center shrink-0">
              <Icon name="event" size={15} className="text-slate-400 dark:text-slate-500" />
            </div>
            <div className="min-w-0">
          ...
          <div className="flex items-center gap-3 px-3.5 py-3">
            <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900/50 flex items-center justify-center shrink-0">
              <Icon name="stethoscope" size={15} className="text-slate-400 dark:text-slate-500" />
            </div>
            <div className="min-w-0">
              <p className="text-[10.5px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Doctor</p>
              <p className="text-[13.5px] font-bold text-slate-900 dark:text-slate-100 truncate">{cita.doctor_nombre}</p>
            </div>
          </div>
        </div>
        ...
        {cita.alergias.length > 0 && (
          <div className="flex items-center gap-3 px-3.5 py-3 bg-rose-50 dark:bg-rose-900/30 border border-rose-100 dark:border-rose-800 rounded-xl">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[12.5px] text-rose-700 dark:text-rose-400 font-semibold">{cita.alergias.join(", ")}</p>
            </div>
          </div>
        )}
        {cita.notas && (
          <div className="flex items-start gap-3 px-3.5 py-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl">
            <Icon name="notes" size={16} className="text-slate-400 dark:text-slate-500 shrink-0 mt-0.5" />
            <span className="text-[12.5px] text-slate-600 dark:text-slate-300 leading-relaxed">{cita.notas}</span>
          </div>
        )}
      </div>
    </ResponsiveSheet>
