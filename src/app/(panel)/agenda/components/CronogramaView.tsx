...
export function CronogramaView({ citas, today, onEventClick, doctorMap }: {
 ...
 <Icon name="schedule_view" size={34} className="text-slate-200"/>
 <p className="text-[13px] text-slate-400">Sin citas registradas</p>
      </div>
    );
  }

  return (
 <div className="h-full overflow-y-auto no-scrollbar px-3 md:px-6 py-4 bg-slate-50">
 ...
 <span className={`text-[12.5px] font-bold capitalize ${isToday ? "text-cyan-700" : "text-slate-700"}`}>
                  {fmtFechaLarga(fecha)}
                </span>
                {isToday && (
 <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-cyan-50 text-cyan-700">Hoy</span>
                )}
              </div>
 ...
 className="w-full text-left rounded-xl border border-slate-200 bg-white hover:border-cyan-300 hover:bg-cyan-50/40 transition-colors px-3.5 py-3 flex items-center gap-3"
                    >
