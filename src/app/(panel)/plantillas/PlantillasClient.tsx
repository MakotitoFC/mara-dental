...
function VisorPlantillaModal({
 ...
 <h2 className="text-[13px] md:text-sm font-bold text-slate-800 truncate">
          ...
          <div className="flex items-center gap-2 shrink-0 pl-4">
            <button
              onClick={handlePrint}
 className="flex items-center justify-center gap-2 px-3 py-1.5 text-[11px] md:text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors"
            >
              <Icon name="print" size={16} /> <span className="hidden sm:inline">Imprimir</span>
            </button>
            <button
              onClick={handleDownload}
 className="flex items-center justify-center gap-2 px-3 py-1.5 text-[11px] md:text-xs font-semibold text-cyan-700 bg-cyan-100 hover:bg-cyan-200 rounded-lg transition-colors"
            >
              <Icon name="download" size={16} /> <span className="hidden sm:inline">Descargar</span>
            </button>
            <button
              onClick={onClose}
 className="w-9 h-9 ml-2 flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors"
            >
        ...
        {/* Contenido */}
 <div className="flex-1 overflow-auto bg-slate-100/50 flex items-center justify-center p-4">
          {isImage ? (
            <img
...
export default function PlantillasClient({ plantillas, userRole }: PlantillasClientProps) {
      ...
      <form id={formId} onSubmit={handleUpload} className="flex flex-col gap-4">
        <div>
          ...
          <input
            ...
            name="file"
            accept=".pdf, image/*"
 className="w-full text-[13px] md:text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[11px] md:file:text-xs file:font-semibold file:bg-cyan-50 file:text-cyan-700 hover:file:bg-cyan-100 border border-slate-200 rounded-xl p-2 bg-slate-50"
            required
          />
        </div>
 ...
 <header className="shrink-0 flex flex-col gap-4 bg-white px-4 sm:px-6 pt-4 sm:pt-6 border-b border-slate-200">
      {/* En mobile solo el título — ícono y descripción se ocultan. */}
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="min-w-0">
 <h1 className="text-[15px] md:text-base font-bold text-slate-800">Formatos y Plantillas</h1>
        ...
        {canUploadDelete && (
          <button
            ...
            className="shrink-0 flex items-center justify-center gap-1.5 h-9 px-3 sm:px-3.5 rounded-lg bg-cyan-600 hover:bg-cyan-700 text-white text-[12.5px] font-semibold transition-colors"
          >
 ...
 <div className="col-span-full py-12 flex flex-col items-center justify-center text-slate-400 bg-white/50 rounded-2xl border border-dashed border-slate-300">
            ...
            filteredPlantillas.map((p) => {
              ...
              return (
 <div key={p.id} className="bg-white border border-slate-200 rounded-2xl flex flex-col hover:shadow-md transition-shadow group relative overflow-hidden">
 ...
 className={`relative h-36 w-full flex items-center justify-center overflow-hidden ${isImage ?'bg-slate-100':'bg-red-50'}`}
                  >
                    {isImage ? (
                      <img src={p.displayUrl} alt={p.nombre_archivo} className="w-full h-full object-cover" />
                    ) : (
 <Icon name="picture_as_pdf" size={48} className="text-red-400"/>
                    )}
                    ...
                    {p.sede_id === null && (
 <span className="absolute top-2 right-2 text-[10px] md:text-[11px] font-bold bg-cyan-100 text-cyan-800 px-2 py-0.5 rounded-full">
                        GLOBAL
                      </span>
                    )}
                  </button>
                  ...
                  <div className="flex flex-col flex-1 p-4 gap-3">
                    <div className="min-w-0">
 <h3 className="text-[13px] md:text-sm font-bold text-slate-800 truncate" title={p.nombre_archivo}>
                        {p.nombre_archivo}
                      </h3>
